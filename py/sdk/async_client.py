...
class R2RAsyncClient(BaseClient):
    ...
    async def _make_request(
        ...
        if (
            ...
            raise R2RException(
                status_code=401,
                message="Access token api key is required to access `https://api.sciphi.ai`. To change the base url, use `set_base_url` method or set the local environment variable `R2R_API_BASE` to `http://localhost:7272`.",
            )
